 ...
 **/
 ...
 //print_r($sql); mostra os comandos anteriores na página e no código fonte.

//Executa a query (método query): 

$conn -> query($sql);
//o método query faz com que os dados sejam inseridos a cada atualização da página.

//READ. O <<<SQL pode ter qualquer coisa escrita, não precisa ter sql escrito estritamente. Usa-se o nome da variável.
$sql = <<<SQL
SELECT * FROM users;
 SQL;  
/*para fazer uma pesquisa filtrada e ordenada, usa-se ORDER BY *a informação que se quer* desc/asc. */


// Executa a query e armazena os dados em $res:
$res = $conn->query($sql);

// print_r($res); mostra os comandos anteriores na página e no código fonte.
/*$user = $res->fetch_assoc();
$user = $res->fetch_assoc(); //ao repetir o comando, ele pega os dados os outros usuários. Se há um loop, a cada volta do loop ela pegará novos dados. */

while ($user = $res->fetch_assoc ()){
print_r($user);} //user é uma array, e todos os dados estão dentro.

// Ao filtrar as informações queridas pelo select, o loop também será filtrado.

//SELECT art_id, art_title, art_thumb, art_intro FROM articles where art_status = 'on' AND art_date <= Now() 
// O "Now" faz uma  busca de algo feito "agora";